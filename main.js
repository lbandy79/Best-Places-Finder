...
async function getVenues() {
  ...
  const urlToFetch = url + city + "&limit=10&client_id=" + clientId + "&client_secret=" + clientSecret + "&v=20180430";
  try {
...
function getValues() {
  ...
  for (let i = 0; i < places.length; i++) {
    for (let j = 0; j < places[i].length; j++) {
      ...
      }).addTo(mymap).bindPopup("<b>" + places[0][j] + "</b><br>" + places[3][j]).openPopup();
    }
